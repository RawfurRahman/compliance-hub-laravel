<div class="bg-white shadow rounded-lg p-6 mb-8">
    ...
    <div x-init="if (!Array.isArray(details.overall_findings)) { details.overall_findings = [] }">
        <table class="min-w-full divide-y divide-gray-300 assessment-table">
            ...
            <tbody class="divide-y divide-gray-200 bg-white">
                <template x-for="(finding, index) in details.overall_findings" :key="index">
                    <tr>
                        ...
                        <td class="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium" x-show="isEditing">
                            <button type="button" @click="details.overall_findings.splice(index, 1)" class="text-red-600 hover:text-red-900">Remove</button>
                        </td>
                    </tr>
                </template>
            </tbody>
        </table>
        <button type="button" @click="details.overall_findings.push({ title: '', description: '', severity: '' })" class="mt-4 px-3 py-1 bg-blue-500 text-white rounded-md text-sm hover:bg-blue-600" x-show="isEditing">
            Add Finding
        </button>
    </div>
</div>
