            ...
            <tbody class="divide-y divide-gray-200 bg-white">
                <template x-for="(scan, index) in details.external_scans" :key="index">
                    <tr>
                        ...
                        <!-- Actions -->
                        <td x-show="isEditing"><button type="button" @click="details.external_scans.splice(index, 1)" class="text-red-600 hover:text-red-900">Remove</button></td>
                    </tr>
                </template>
            </tbody>
        </table>
        <button type="button" @click="details.external_scans.push({ scan_date: '', result: '', initial_assessment: false })" class="mt-2 text-blue-600 hover:text-blue-900" x-show="isEditing">Add External Scan</button>
    </div>
            ...
            <tbody class="divide-y divide-gray-200 bg-white">
                <template x-for="(scan, index) in details.internal_scans" :key="index">
                    <tr>
                        ...
                        <!-- Actions -->
                        <td x-show="isEditing"><button type="button" @click="details.internal_scans.splice(index, 1)" class="text-red-600 hover:text-red-900">Remove</button></td>
                    </tr>
                </template>
            </tbody>
        </table>
        <button type="button" @click="details.internal_scans.push({ scan_date: '', result: '', initial_assessment: false })" class="mt-2 text-blue-600 hover:text-blue-900" x-show="isEditing">Add Internal Scan</button>
    </div>
</div>
