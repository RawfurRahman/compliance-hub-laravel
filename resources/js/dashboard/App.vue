<template>
  ...
  >
    <template #kpis>
      <KpiCard label="Active Projects" :value="kpi.data.value?.projects" color="blue" :loading="kpi.loading.value" :error="kpi.error.value">
      ...
      <KpiCard label="Frameworks" :value="kpi.data.value?.frameworks" color="purple" :loading="kpi.loading.value" :error="kpi.error.value">
      ...
      <KpiCard label="Finding Compliance" :value="kpi.data.value?.compliance_pct" subtitle="of findings compliant" color="emerald" :loading="kpi.loading.value" :error="kpi.error.value">
      ...
      <KpiCard label="Overdue" :value="kpi.data.value?.overdue_findings" subtitle="findings past due" color="red" :trend-down="(kpi.data.value?.overdue_findings ?? 0) > 0" :loading="kpi.loading.value" :error="kpi.error.value">
    ...
    <template #heatmap>
      <RiskHeatmapGrid :cells="heatmap.data.value ?? []" :loading="heatmap.loading.value" :error="heatmap.error.value" />
    </template>

    <template #inherent-residual>
      <InherentResidualBarChart :items="inherentResidual.data.value ?? []" :loading="inherentResidual.loading.value" :error="inherentResidual.error.value" />
    </template>

    <template #control-effectiveness>
      <ControlEffectivenessDonut
        :effective="controlEffectiveness.data.value?.effective ?? 0"
        :partial="controlEffectiveness.data.value?.partial ?? 0"
        :ineffective="controlEffectiveness.data.value?.ineffective ?? 0"
    ...
    <template #audit-findings>
      <AuditFindingsDonut :data="auditFindings.data.value?.severity_breakdown ?? {}" :loading="auditFindings.loading.value" :error="auditFindings.error.value" />
    </template>

    <template #third-party-risk>
      <ThirdPartyRiskDonut
        :breakdown="thirdPartyRisk.data.value?.risk_tier_breakdown ?? {}"
        :total-vendors="thirdPartyRisk.data.value?.total_vendors ?? 0"
    ...
    <template #top-risks>
      <TopRisksTable :rankings="riskRankings.data.value ?? []" :loading="riskRankings.loading.value" :error="riskRankings.error.value" />
    </template>

    <template #issue-aging>
      <IssueAgingBarChart :buckets="issueAging.data.value?.buckets ?? {}" :loading="issueAging.loading.value" :error="issueAging.error.value" />
    </template>

    <template #remediation-trend>
      <RemediationTrendChart :monthly="remediationTrend.data.value?.monthly_trend ?? []" :loading="remediationTrend.loading.value" :error="remediationTrend.error.value" />
    </template>

    <template #compliance-scorecard>
      <ComplianceScorecardTable :scorecard="complianceScorecard.data.value ?? []" :loading="complianceScorecard.loading.value" :error="complianceScorecard.error.value" />
    </template>
  </DashboardShell>
</template>
...
const isRefreshing = computed(() =>
  [kpi, heatmap, riskRankings, inherentResidual, controlEffectiveness,
   ...
   thirdPartyRisk].some(a => a.loading.value)
)
...
function refreshAll() {
  ;[kpi, heatmap, riskRankings, inherentResidual, controlEffectiveness,
    complianceScorecard, auditFindings, remediationTrend, issueAging,
    thirdPartyRisk].forEach(a => a.refresh())
}
</script>
