...
<div class="bg-white shadow rounded-lg p-6 mb-8">
    ...
    <div x-data="{ activities: {{ json_encode(optional($details)->assessment_activities ?? []) }} }">
                ...
                <template x-for="(activity, index) in activities" :key="index">
                    <tr>
                        ...
                        <td class="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium" x-show="isEditing">
                            <button type="button" @click="activities.splice(index, 1)" class="text-red-600 hover:text-red-900">Remove</button>
                        </td>
                    </tr>
                </template>
            </tbody>
        </table>
        <button type="button" @click="activities.push({ name: '', date: '', outcome: '' })" class="mt-4 px-3 py-1 bg-blue-500 text-white rounded-md text-sm hover:bg-blue-600" x-show="isEditing">
            Add Activity
        </button>
    </div>
</div>
