...
<div class="bg-white shadow rounded-lg p-6 mb-8">
    ...
    <div class="space-y-6">
        <div x-data="{ segmentationUsed: {{ optional($details)->segmentation_used ? 'true' : 'false' }} }">
            ...
            <div class="flex items-center mt-2" x-show="isEditing">
                <input type="radio" name="segmentation_used" id="segmentation_used_yes" value="1" x-model="segmentationUsed" class="h-4 w-4 text-blue-600 border-gray-300">
                <label for="segmentation_used_yes" class="ml-2 text-sm">Yes</label>
                <input type="radio" name="segmentation_used" id="segmentation_used_no" value="0" x-model="segmentationUsed" class="ml-4 h-4 w-4 text-blue-600 border-gray-300">
                <label for="segmentation_used_no" class="ml-2 text-sm">No</label>
            </div>
            <div x-show="!isEditing" class="mt-1 font-semibold">{{ optional($details)->segmentation_used ? 'Yes' : 'No' }}</div>
            <div x-show="segmentationUsed" x-transition class="mt-2">
                <label for="segmentation_desc" class="block text-sm font-medium text-gray-700">Description of Segmentation</label>
                <div x-show="!isEditing" class="mt-1 p-2 bg-gray-50 rounded-md border">{{ optional($details)->segmentation_desc ?? 'No description provided.' }}</div>
        ...
        <hr class="my-6">
        <div x-data="{ productsUsed: {{ optional($details)->pci_ssc_products_used ? 'true' : 'false' }}, products: {{ json_encode(optional($details)->pciSscProducts ?? []) }} }">
            <label class="block text-sm font-medium text-gray-700">Are PCI SSC-validated products or solutions used?</label>
             <div class="flex items-center mt-2" x-show="isEditing">
                <input type="radio" name="pci_ssc_products_used" id="pci_ssc_products_used_yes" value="1" x-model="productsUsed" class="h-4 w-4 text-blue-600 border-gray-300">
                <label for="pci_ssc_products_used_yes" class="ml-2 text-sm">Yes</label>
                <input type="radio" name="pci_ssc_products_used" id="pci_ssc_products_used_no" value="0" x-model="productsUsed" class="ml-4 h-4 w-4 text-blue-600 border-gray-300">
                <label for="pci_ssc_products_used_no" class="ml-2 text-sm">No</label>
            </div>
            <div x-show="!isEditing" class="mt-1 font-semibold">{{ optional($details)->pci_ssc_products_used ? 'Yes' : 'No' }}</div>
            <div x-show="productsUsed" x-transition class="mt-4">
                    ...
                    <tbody class="divide-y divide-gray-200 bg-white">
                        <template x-for="(product, index) in products" :key="index">
                            <tr>
                                ...
                                <td x-show="isEditing"><button type="button" @click="products.splice(index, 1)" class="text-red-600">Remove</button></td>
                            </tr>
                        </template>
                    </tbody>
                </table>
                <button type="button" @click="products.push({ product_name: '', version: '', vendor: '', description: '' })" class="mt-2 text-blue-600" x-show="isEditing">Add Product</button>
            </div>
        </div>
    </div>
</div>
